...
def predict_potability_p(request):
    if request.method == 'POST':
        ...
        if result == 1:
            ...
            if 6.5 <= sum_of_user_input <= 8.5 :
                useability = "Drinkable"
            elif 6.0 <= sum_of_user_input <= 8.5:
                useability = "Domestic"
            elif 6.0 <= sum_of_user_input <= 9.0:
...
def predict_potability(request):
    if request.method == 'POST':
        ...
        if Turbidity < 14:
            prediction = "The water is potable."
            if 6.5 <= pH <= 8.5: 
                useability = "Water is safe to be used for drinkable purposes."
            elif 6.0 <= pH <= 8.5: 
                useability =  "Water is suitable for domestic use."
            elif 6.0 <= pH <= 9.0: 
                useability =  "Water is good for agricultural use."
            # else:
            #     useability = "Groundwater quality may be compromised. Further testing advised."
                        
        else:
            prediction = "The water is not potable. pump the water into the Ground"
...
def read_sensor_data(request):
    if request.method == 'GET':
        ...
        ser = serial.Serial('COM8', 9600)  # Replace 'COM4' with your serial port
        ser.flushInput()
